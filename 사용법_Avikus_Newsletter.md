...
```
newsletter_new/
├─ create_outlook_email_issue01.py   ★ 실행 파일 — Outlook 새 메일 창을 띄움
├─ newsletter_email_issue01.html       메일 본문(디자인/글). 내용 수정은 여기
...
├─ archive_vol1/           이전 HiNAS Vol.1 버전 파일 보관(참고용)
└─ _source/                원본 디자인 소스(Monthly Newsletter.pptx) 보관
```

> 이미지는 HTML의 `cid:이름` 과 스크립트(`create_outlook_email_issue01.py`)의 `IMAGES` 딕셔너리 키가
> **이름으로 연결**됩니다. 둘의 이름이 같아야 본문에 표시됩니다.
...
1. 파일 탐색기에서 이 폴더 주소창에 `cmd` 를 입력하고 Enter → 이 폴더에서 명령 프롬프트가 열립니다.
2. 아래 명령 실행:

```
py create_outlook_email_issue01.py
```
...
| 바꾸고 싶은 것 | 수정할 곳 |
|---|---|
| 글, 문구, 레이아웃 | `newsletter_email_issue01.html` 편집 후 다시 실행 |
| 이미지 교체 | `assets/` 안의 **같은 이름** 파일로 교체 (자동 반영) |
| 새 이미지 추가 | HTML에 `<img src="cid:새이름">` 추가 + 스크립트 `IMAGES` 에 `"새이름": "파일.png"` 등록 |
| 제목 / 받는 사람 / 발송 방식 | `create_outlook_email_issue01.py` (아래 5장) |
...
`create_outlook_email_issue01.py` 파일을 편집기로 열어 아래 부분을 수정합니다.
...
| 사진 모서리가 각져 보임 | Outlook 데스크톱은 CSS 둥근 모서리를 무시함 → 이미지 파일 자체에 둥근 모서리를 입혀야 함(이미 적용됨, 원본은 `assets/_orig/`) |
...
- 가공 전 **원본은 `assets/_orig/`** 에 백업되어 있습니다.
- 이미지 가공에는 추가 패키지가 필요합니다: `py -m pip install pillow`

---

*이 문서를 Word(.docx)로 다시 만들려면: `py build_docx.py`*
