...
## 3-2. 대량 발송 (선사 등 다수 수신자에게 BCC 한 통)

250명처럼 많은 수신자에게 **한 통으로(서로 주소 안 보이게 BCC)** 보낼 때 사용합니다.

1. 수신자 엑셀을 준비합니다. **열 위치·제목은 상관없습니다** — 어느 칸이든
   `a@b.com` 형태면 스크립트가 자동으로 다 찾아냅니다.
2. 엑셀에서 **`다른 이름으로 저장 ▸ CSV UTF-8(쉼표로 분리)`** 로 저장해
   저장소 최상위 **`recipients/`** 폴더에 넣습니다. (`.xlsx` 그대로 넣어도 됨)
3. 발송할 호 폴더에서:
...
→ 추출 결과(고유 인원수 / 중복 제거 수)가 출력되고,
→ 전원이 **BCC** 에 들어간 Outlook 새 메일 창이 열립니다.
→ **BCC 인원수를 확인한 뒤** 직접 **보내기**.

> - 이 폴더 안의 **모든** CSV/XLSX/TXT 를 읽어 합치고 **중복 제거**(대소문자 무시)합니다.
> - 한 통 수신자가 너무 많으면(약 450명 초과) 경고가 뜹니다 — Exchange 정책상 거부될 수 있어 목록을 나눠 보내세요.
> - **실제 수신자 목록은 개인정보라 git 에 커밋되지 않습니다**(`.gitignore` 제외). 자세한 규칙은 `recipients/README.md` 참고.

> ⚠️ 대량 발송도 **6장 발송 주의사항**(옛 초안창 닫기·Outbox 확인)이 그대로 적용됩니다.
